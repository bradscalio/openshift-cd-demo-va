...
function usage() {
    ...
    echo "   --private                  Optional    Get all templates from the local repo" 
    echo
}
...
REPO_NAME=${REPO_NAME:-openshift-cd-demo}
...
function deploy() {
  ...
  # add EAP images
  oc create -f $CICD_TEMPLATE_LOCATION/eap/eap70-image-stream.json -n dev-$PRJ_SUFFIX 
